...
public class NumberParamTemplate extends FlowParamJavaCodeTemplate {

    public NumberParamTemplate(FlowParam flowParam, String parentPointerChain) {
    ...
    public List<String> javaCode(boolean needAutoMapping) {
        return Collections.singletonList(
                String.format("Number %s = %s", pointer(), DATA_TRANSFER(PARAM_VALUE(needAutoMapping)))
        );
    }

    @Override
    public List<DataTypeEnum> acceptDataType() {
        return Collections.singletonList(DataTypeEnum.NUMBER);
    }
}
